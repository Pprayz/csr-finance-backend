...
async function createSchema() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS training_enrollments (
      ...
      employee_id TEXT REFERENCES employees(id),
      status TEXT DEFAULT 'en attente' CHECK(status IN ('en attente','approuvée','refusée','terminée')),
...
async function runMigrations() {
  ...
  await pool.query(`ALTER TABLE entities ADD COLUMN IF NOT EXISTS active INTEGER DEFAULT 1;`);
  ...
  `);
  await pool.query(`ALTER TABLE trainings ADD COLUMN IF NOT EXISTS modules JSONB;`);
  await pool.query(`
    CREATE TABLE IF NOT EXISTS training_enrollments (
      id TEXT PRIMARY KEY,
      training_id TEXT REFERENCES trainings(id) ON DELETE CASCADE,
  ...
  `);
  await pool.query(`ALTER TABLE training_enrollments ADD COLUMN IF NOT EXISTS completed_modules JSONB DEFAULT '[]';`);
  await pool.query(`ALTER TABLE trainings ADD COLUMN IF NOT EXISTS quiz JSONB;`);
  await pool.query(`ALTER TABLE training_enrollments ADD COLUMN IF NOT EXISTS quiz_score INTEGER;`);
  await pool.query(`ALTER TABLE training_enrollments ADD COLUMN IF NOT EXISTS quiz_passed INTEGER DEFAULT 0;`);
  await pool.query(`ALTER TABLE training_enrollments ADD COLUMN IF NOT EXISTS started_at TIMESTAMP;`);
  await pool.query(`ALTER TABLE trainings ADD COLUMN IF NOT EXISTS attachment_name TEXT;`);
  await pool.query(`ALTER TABLE trainings ADD COLUMN IF NOT EXISTS attachment_data TEXT;`);
  await pool.query(`
    CREATE TABLE IF NOT EXISTS quotes (
      id TEXT PRIMARY KEY,
      entity_id TEXT REFERENCES entities(id),
