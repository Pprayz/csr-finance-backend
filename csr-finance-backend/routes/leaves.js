...
// Modification complète (réservée aux directeurs, toutes filiales)
router.patch('/:id', async (req, res) => {
  ...
  if (!leave) return res.status(404).json({ error: 'Demande introuvable' });
  await db.run(
    'UPDATE leaves SET type = $1, start_date = $2, end_date = $3, status = $4 WHERE id = $5',
    [type || leave.type, start || leave.start_date, end || leave.end_date, status || leave.status, req.params.id]
  );
  res.json({ ok: true });
});
