...
router.get('/', async (req, res) => {
  const cols = 'id, entity_id, kind, number, counterparty, amount, due, status, attachment_name, items';
...
router.post('/', async (req, res) => {
  const { entityId, kind, number, counterparty, amount, due, attachmentName, attachmentData, items } = req.body;
  ...
  await db.run(
    `INSERT INTO invoices (id,entity_id,kind,number,counterparty,amount,due,status,attachment_name,attachment_data,items)
     VALUES ($1,$2,$3,$4,$5,$6,$7,'en attente',$8,$9,$10)`,
    [id, targetEntity, kind, number, counterparty, amount, due, attachmentName || null, cleanedData || null, items ? JSON.stringify(items) : null]
  );
