...
function trainingOut(t) {
  ...
  return { id: t.id, title: t.title, description: t.description, duration: t.duration, category: t.category, modules: t.modules || [], quiz, hasQuiz: quiz.length > 0, active: t.active === 1 || t.active === true };
}
...
router.post('/', async (req, res) => {
  ...
  const { title, description, duration, category, modules, quiz } = req.body;
  if (!title) return res.status(400).json({ error: 'Le titre est requis' });
  const id = 'fo' + Date.now();
  const modulesToStore = cleanModules(modules);
  await db.run('INSERT INTO trainings (id,title,description,duration,category,modules,quiz) VALUES ($1,$2,$3,$4,$5,$6,$7)',
    [id, title, description || null, duration || null, category || null, JSON.stringify(modulesToStore), JSON.stringify(cleanQuiz(quiz))]);
...
router.patch('/:id', async (req, res) => {
  ...
  const { title, description, duration, category, active, modules, quiz } = req.body;
  const newModules = modules !== undefined ? cleanModules(modules) : (t.modules || []);
  const newQuiz = quiz !== undefined ? cleanQuiz(quiz) : (t.quiz || []);
  await db.run(
    'UPDATE trainings SET title=$1, description=$2, duration=$3, category=$4, active=$5, modules=$6, quiz=$7 WHERE id=$8',
    [title || t.title, description !== undefined ? description : t.description, duration !== undefined ? duration : t.duration,
      category !== undefined ? category : t.category, active !== undefined ? (active ? 1 : 0) : t.active, JSON.stringify(newModules), JSON.stringify(newQuiz), req.params.id]
  );
  res.json({ ok: true });
});
