...
// "spent" = somme des dépenses du mois en cours liées à cette carte (calculé, jamais stocké manuellement)
const SPENT_SELECT = `
  SELECT c.*, COALESCE((
    SELECT SUM(-t.amount) FROM transactions t
    WHERE t.card_id = c.id AND t.amount < 0
...
router.get('/', async (req, res) => {
  const rows = req.entityScope
    ? await db.all(SPENT_SELECT + ' WHERE c.entity_id = $1', [req.entityScope])
    : await db.all(SPENT_SELECT);
  res.json(rows);
});
