...
function employeeOut(e) {
  return {
    ...
    contractPdfName: e.contract_pdf_name,
  };
}
...
// Génère/remplace le contrat de travail (PDF) — réservé aux directeurs
router.post('/employees/:id/contract', requireAuth, async (req, res) => {
  ...
  if (!attachmentData) return res.status(400).json({ error: 'Fichier manquant' });
  const commaIndex = attachmentData.indexOf(',');
  const cleaned = attachmentData.startsWith('data:') && commaIndex !== -1 ? attachmentData.slice(commaIndex + 1) : attachmentData;
  await db.run('UPDATE employees SET contract_pdf_name = $1, contract_pdf_data = $2 WHERE id = $3', [attachmentName || 'contrat.pdf', cleaned, req.params.id]);
  res.json({ ok: true });
});

// Télécharge le contrat : directeur, responsable de l'entité concernée, ou le salarié lui-même
router.get('/employees/:id/contract', requireAuth, async (req, res) => {
  const emp = await db.get('SELECT * FROM employees WHERE id = $1', [req.params.id]);
  if (!emp) return res.status(404).json({ error: 'Salarié introuvable' });
  const isSelf = req.user.employeeId === emp.id;
  const isResponsableHere = req.user.role === 'responsable' && req.user.entityId === emp.entity_id;
  if (req.user.role !== 'directeur' && !isResponsableHere && !isSelf) return res.status(403).json({ error: 'Accès non autorisé' });
  if (!emp.contract_pdf_data) return res.status(404).json({ error: 'Aucun contrat généré pour ce salarié' });
  const buffer = Buffer.from(emp.contract_pdf_data, 'base64');
  res.setHeader('Content-Type', 'application/pdf');
  res.setHeader('Content-Disposition', `inline; filename="${(emp.contract_pdf_name || 'contrat.pdf').replace(/"/g, '')}"`);
  res.send(buffer);
});
